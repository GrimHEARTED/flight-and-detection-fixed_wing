'''the flight-control node implemented on the companion computer (based on flight.py)'''
'''gonna be replaced with the one based on flight_circle.py'''
import rclpy
...
from px4_msgs.msg import VehicleCommand, OffboardControlMode, VehicleLocalPosition,VehicleAttitudeSetpoint, \
                            VehicleLocalPositionSetpoint, TrajectorySetpoint,ActuatorServos
...
class PX4Whisperer(Node):
    ...
    def do_find_another(self):
        ...
        if self.search_flag == 0 :
            ...
            pos_ref.velocity = [float('nan')] * 3
            rad1 = self.init_yaw + 0.01 - self.search_flag*math.pi/3
            pos_ref.position[0] = self.startpoint[0] + 80 * math.cos(rad1) + 50 * math.cos(rad2)
            pos_ref.position[1] = self.startpoint[1] + 80 * math.sin(rad1) + 50 * math.sin(rad2)
            pos_ref.position[2] = -22.0
            pos_ref.yaw = math.nan
            self.traj_ref_pub.publish(pos_ref)
        elif self.search_flag == 3:
            rad2 = self.init_yaw + 0.01 - math.pi/2
            pos_ref.velocity = [float('nan')] * 3
    ...
    def do_attack(self):
        ...
        if self.attack_flag == 0:
            pos_ref.velocity = [float('nan')] * 3
            pos_ref.position[0] = self.result_x + 120 * math.cos(self.init_yaw) + 80 * math.cos(rad1)
            pos_ref.position[1] = self.result_y + 120 * math.sin(self.init_yaw) + 80 * math.sin(rad1)
