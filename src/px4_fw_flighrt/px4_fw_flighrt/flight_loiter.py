'''flight simulation with a track over the target area in many differnet direction'''
'''mainly used for data collection'''
...
class PX4Whisperer(Node):
    ...
    def do_forward(self):
        ...
        pos_ref.velocity[0] = math.cos(self.init_yaw-0.04)*8
        pos_ref.velocity[1] = math.sin(self.init_yaw-0.04)*8
    ...
    def do_find_another(self):
        ...
        if self.search_flag == -1 :
            ...
            pos_ref.position[0] = self.startpoint[0] + self.vx * 5
            pos_ref.position[1] = self.startpoint[1] + self.vy * 5
            pos_ref.position[2] = -22.0
            pos_ref.yaw = math.nan
            self.traj_ref_pub.publish(pos_ref)
        if self.search_flag%6 == 0 :
            pos_ref.velocity = [float('nan')] * 3
            rad1 = self.init_yaw + (self.search_flag/1.5)*math.pi/12 + math.pi/10
            pos_ref.position[0] = self.startpoint[0] + 180 * math.cos(rad1)
            pos_ref.position[1] = self.startpoint[1] + 180 * math.sin(rad1)
            pos_ref.position[2] = -22.0
            pos_ref.yaw = math.nan
            self.traj_ref_pub.publish(pos_ref)
        elif self.search_flag%6 == 3:
            pos_ref.velocity = [float('nan')] * 3
            rad1 = self.init_yaw - math.pi + (self.search_flag/1.5)*math.pi/12 + math.pi/10
            pos_ref.position[0] = self.startpoint[0] + 180 * math.cos(rad1)
            pos_ref.position[1] = self.startpoint[1] + 180 * math.sin(rad1)
            pos_ref.position[2] = -22.0
            pos_ref.yaw = math.nan
            self.traj_ref_pub.publish(pos_ref)
        elif self.search_flag%6 == 1:
            pos_ref.velocity = [float('nan')] * 3
        ...
        if self.reach_or_not(pos_ref.position[0], pos_ref.position[1],pos_ref.position[2],64.0):
            self.search_flag = self.search_flag + 1
            self.get_logger().info(' ==============Next Search Flag: "%i"=================' % self.search_flag)
    ...
    def do_attack(self):
        ...
        if self.attack_flag == 0:
            pos_ref.velocity = [float('nan')] * 3
            pos_ref.position[0] = self.result_x + 130 * math.cos(self.init_yaw) + 60 * math.cos(rad1)
            pos_ref.position[1] = self.result_y + 130 * math.sin(self.init_yaw) + 60 * math.sin(rad1)
            ...
            pos_ref.velocity = [float('nan')] * 3
            pos_ref.position[0] = self.result_x + 100 * math.cos(self.init_yaw)
            pos_ref.position[1] = self.result_y + 100 * math.sin(self.init_yaw)
            ...
            pos_ref.yaw = math.nan
            self.traj_ref_pub.publish(pos_ref)
            if self.reach_or_not(self.result_x, self.result_y,-5.0,250.0):
                self.attack_flag += 1
        elif self.attack_flag == 3:
            pos_ref.velocity = [float('nan')] * 3
            pos_ref.position[0] = self.result_x
            pos_ref.position[1] = self.result_y
            pos_ref.position[2] = -5.0
            pos_ref.yaw = math.nan
            self.traj_ref_pub.publish(pos_ref)
            threshold = (self.vx**2+self.vy**2)*1.2
    ...
    def do_return(self):
        ...
        if self.z < -0.01:
            if self.return_flag == 0:
                setpoint.position = [0.0,0.0,0.6]
                setpoint.yaw = math.nan
                setpoint.velocity = [ 
                    #math.cos(self.init_yaw-math.pi)*8,
                    #math.sin(self.init_yaw-math.pi)*8,
                    math.nan,
                    math.nan,
                    math.nan
                ]
                ...
                setpoint.position = [math.nan,math.nan,0.0]
                setpoint.yaw = math.nan
                setpoint.velocity = [ 
                    self.vx*0.1,
                    self.vx*0.1,
