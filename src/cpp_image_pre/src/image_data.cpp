...
/*
虚拟视觉目标检测节点: 负责从/camera话题订阅图像，通过 HSV 颜色空间提取特定颜色特征，
...
public:
	...
	// 2. 创建发布者与订阅者
        center_pub_ = this->create_publisher<vision_msgs::msg::Point2D>("center", qos_profile); 
        image_pub_  = this->create_publisher<sensor_msgs::msg::Image>("nums", qos_profile); 
        ...
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(       
            "camera", qos_profile, std::bind(&TargetDetectionNode::get_image, this, _1));
...
private:
    ...
    Mat frame;
    short status_counter_ = 0;
    short process_counter_ = 0;
    bool is_active_ = false;    // 节点是否处于活跃工作状态
    bool is_processing_ = false;// 防止重入导致的处理冲突
    ...
    void process_frame()
    {
        ...
        if (++process_counter_ % 30 == 0) {
            RCLCPP_INFO(this->get_logger(), "Processing Time: '%.2f' ms", elapsed_time.count());  
            status_counter_ = 0;
        }
	    is_processing_ = false;
    }
